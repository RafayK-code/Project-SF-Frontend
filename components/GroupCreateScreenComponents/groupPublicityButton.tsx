...
function GroupPublicityButton(props: GroupPublicityProps) {
    return (
        <>
            <TouchableOpacity style={[styles.container, props.selected ? styles.selected : undefined]} onPress={() => { props.onClick(props.selected) }} activeOpacity={0.7} >

                <Text style={[styles.text, props.selected ? styles.selected : undefined]}>{props.text}</Text>
                <View style={[styles.imageCont, props.selected ? styles.selectedUpper : undefined]}>
                    {
                        props.open ? <PublicIcon color={props.selected ? "#3D404A" : "#F7F3E8"}></PublicIcon> : <PrivateIcon color={props.selected ? "#3D404A" : "#F7F3E8"}></PrivateIcon>
                    }
                </View>
            </TouchableOpacity>
        </>
